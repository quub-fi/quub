...
// Quub / custom precompile factories: same OpExecData bridge for PostExecEvmFactoryAdapter.
impl<ChainSpec, N, R, F> reth_evm::ConfigureEngineEvm<OpExecData>
    for reth_optimism_evm::OpEvmConfig<
        ChainSpec,
        N,
        R,
        reth_optimism_evm::PostExecEvmFactoryAdapter<F>,
    >
where
    N: NodePrimitives,
    R: Send + Sync + Unpin + Clone + 'static,
    ChainSpec: Send + Sync + Unpin + Clone + 'static,
    F: Send + Sync + Unpin + Clone + 'static,
    Self: reth_evm::ConfigureEngineEvm<OpExecutionData>,
{
    fn evm_env_for_payload(
        &self,
